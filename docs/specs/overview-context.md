...
## Role of the system
- To receive and store runtime telemetry events generated by a web app
- To offer a summary-analysis capability - over the stored telemetry events
...
### Handle an OPTIONS request for CORS requests

Handle an OPTIONS request with a CORS response suitable for requests coming from browsers
...
## The AnalysisReport data type

```
type AnalysisReport struct {
  // The fields inside the HowManyPeople map are counters. For example
  // "LoadedAnExample" is a count of how unique ProxyUserID's have generated a "launched" event.
  "HowManyPeopleHave": {
    "Launched": 0,
    "LoadedAnExample": 0,
    "TriedToSignIn": 0,
